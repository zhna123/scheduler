import { cancelOnSchedule } from "@/app/lib/actions";
import { Device } from "../lib/data-utils";

export default function CancelOnForm({device, location}: {device: Device, location: string}) {
  return (
    <form action={cancelOnSchedule} className="text-sch_gray-800 w-[350px] flex flex-col h-full">
      <h2 className="mb-10">Are you sure to cancel the current <span className="text-sch_orange-600">{'turn on'}</span> schedule for device:</h2>
